...
class grad_cam_cwox():
    ...
    Produces cluster heatmaps with **Grad-CAM** as base explainer. The back-propagation target is changed to: 
    ...
    Output:
    saliency (Array) - a 2D array of shape (H,W) that represents the saliency value in each pixel position.
    """
    ...
    def __call__(self, image, targets):
        """Call function for `grad_cam_cwox`."""
        poss=targets
        probs = self.model(image).softmax(-1)
        grad = torch.zeros_like(probs)
        poss_p = probs[0, poss]
        grad[0, poss] = poss_p / poss_p.sum()
        saliency = grad_cam(self.model, image, grad, saliency_layer=self.layer,resize = True)[0,0]
