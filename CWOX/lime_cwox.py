...
class LimeBase(object):
    ...
    def explain_instance_with_data(self,
        ...
        # If there is more than 1 class in the cluster, 
        # take the average of the prediction probabilities for the classes in cluster as sign of explanation    
        if len(pos_labels)>1:
...
class lime_cwox():
    r"""
    Produces cluster heatmaps with **LIME** as base explainer. The predicted target is changed to: 
    ...
    Inputs:
        ...
        targets (List of List of Int) - The clusters to be explained. Since RISE itself supports the multiple outputs for different target classes, the converted cluster version also supports that. e.g. [[0,1,2], [3,4,5]] means the cluster that includes class 0,1,2 and cluster that includes class 3,4,5 are explained.
    Output:
        a 3D array of shape (len(target class),H,W) that represents the saliency value in each pixel position for different clusters.
    """
    ...
    def __call__(self, img_path, targets):
        ...
        for class1 in targets:
            sal_dict[str(class1)]=explanation.local_exp[str(class1)]
